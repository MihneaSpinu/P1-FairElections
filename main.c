...
int main() {
    ...
    if(electoral_choice == 1) {
        ...
        strcpy(loser, candidate_arr[0].general_mandates < candidate_arr[1].general_mandates ? candidate_arr[0].name : candidate_arr[1].name);
    ...
    if(simulation_choice == 4 && electoral_choice == 1 && voting_system_choice == 2) {
        for(int i = 0; i < STATES; i++) {
            ...
            int rcv_winner = ranked_choice_voting(state_arr[i].population, voter_arr, candidate_arr,
                                                  start_index[i], &state_arr[i], candidates);
